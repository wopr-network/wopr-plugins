...
export class Qwen3Provider implements TTSProvider {
  readonly metadata: VoicePluginMetadata = {
    ...
    version: "1.0.0",
    type: "tts",
    description: "Qwen3-TTS with voice cloning, voice design, and multilingual support",
    capabilities: ["voice-selection", "voice-cloning", "voice-design", "multilingual", "streaming"],
    local: true,
    emoji: "🗣️",
  };
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (Array.isArray(data)) {
          ...
          const merged = [...this.voices.filter((v) => !dynamicIds.has(v.id)), ...this.dynamicVoices];
          this._voices = merged;
        }
      }
    } catch {
  ...
  async init(context: WOPRPluginContext) {
    ...
    try {
      ...
      if (healthy) {
        ...
        ctx.log.info(`Qwen3 TTS registered (${config.serverUrl ?? DEFAULT_CONFIG.serverUrl})`);
      } else {
        ctx.log.warn(`Qwen3 server not reachable at ${config.serverUrl ?? DEFAULT_CONFIG.serverUrl}`);
      }
    } catch (error: unknown) {
