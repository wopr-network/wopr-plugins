...
function makeMockCtx(overrides: Partial<WOPRPluginContext> = {}): WOPRPluginContext {
  return {
    log: {
      info: vi.fn(),
      warn: vi.fn(),
...
describe("plugin lifecycle", () => {
  ...
  it("init calls registerExtension with the TTS provider", async () => {
    const { default: plugin, ElevenLabsTTSProvider } = await import("../index.js");
    const registerExtension = vi.fn();
    const mockCtx = makeMockCtx({ registerExtension });

    await plugin.init?.(mockCtx);
    expect(registerExtension).toHaveBeenCalledWith("tts", expect.any(ElevenLabsTTSProvider));
  ...
  it("init registers capability provider when available", async () => {
    ...
    await plugin.init?.(mockCtx);
    expect(registerCapabilityProvider).toHaveBeenCalledWith("tts", expect.objectContaining({ id: "elevenlabs" }));
  ...
  it("init registers config schema", async () => {
    ...
    expect(registerConfigSchema).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        fields: expect.arrayContaining([expect.objectContaining({ name: "apiKey", secret: true })]),
      }),
    );
...
describe("ElevenLabsTTSProvider", () => {
  ...
  it("throws if no API key provided", async () => {
    ...
    expect(() => new ElevenLabsTTSProvider({})).toThrow("ELEVENLABS_API_KEY is required");
  });
