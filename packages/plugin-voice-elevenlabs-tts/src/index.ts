 ...
 */
function validateStability(value: number | undefined, modelId?: string): number | undefined {
  if (value === undefined) return undefined;
  if (modelId === "eleven_v3") {
    const valid = [0.0, 0.5, 1.0];
    const closest = valid.reduce((prev, curr) => (Math.abs(curr - value) < Math.abs(prev - value) ? curr : prev));
    return closest;
  }
 ...
 */
function mergeOptions(base: TTSOptions | undefined, extended: ElevenLabsTTSOptions | undefined): ElevenLabsTTSOptions {
  return {
...
export class ElevenLabsTTSProvider implements TTSProvider {
  readonly metadata: VoicePluginMetadata = {
    ...
    version: "1.0.0",
    type: "tts",
    description: "ElevenLabs high-quality text-to-speech with streaming support",
    capabilities: ["streaming", "voice-selection", "voice-parameters", "voice-cloning"],
    local: false,
    requires: {
  ...
  async fetchVoices(): Promise<Voice[]> {
    const now = Date.now();
    if (this.cachedVoices.length > 0 && now - this.voicesCachedAt < this.VOICE_CACHE_TTL) {
      return this.cachedVoices;
    }
   ...
   */
  private async getOrCreateClonedVoice(referenceAudio: Buffer): Promise<string> {
    ...
    formData.append("files", new Blob([audioArrayBuffer], { type: "audio/wav" }), "reference.wav");
    ...
    if (!response.ok) {
      ...
      throw new Error(`ElevenLabs voice cloning failed: ${response.statusText} - ${error}`);
    }
      ...
      if (oldestKey !== undefined) {
        ...
        if (evictedVoiceId) {
          this.deleteClonedVoice(evictedVoiceId).catch((err: Error) => {
            this.logger?.warn(`[ElevenLabs] Failed to delete evicted cloned voice ${evictedVoiceId}: ${err.message}`);
          });
        }
      }
    }
  ...
  private async deleteClonedVoice(voiceId: string): Promise<void> {
    ...
    if (!response.ok) {
      const error = await response.text();
      throw new Error(`ElevenLabs voice delete failed: ${response.statusText} - ${error}`);
    }
  }
  ...
  async synthesize(text: string, options?: TTSOptions): Promise<TTSSynthesisResult> {
    ...
    if (unknownKeys.length > 0) {
      ctx?.log.warn(`[ElevenLabs] Unknown directive keys: ${unknownKeys.join(", ")}`);
    }
    ...
    if (extOpts?.referenceAudio) {
      const clonedVoiceId = await this.getOrCreateClonedVoice(extOpts.referenceAudio);
    ...
    const modelId = opts.modelId || this.config.defaultModelId || "eleven_turbo_v2_5";
    ...
    const requestBody: ElevenLabsTTSRequest = {
      ...
      model_id: modelId,
      voice_settings: {
        stability: validateStability(opts.stability ?? this.config.stability, modelId),
        similarity_boost: validateUnit(opts.similarity ?? this.config.similarityBoost),
    ...
    if (opts.latencyTier !== undefined) {
      queryParams.set("optimize_streaming_latency", String(validateLatencyTier(opts.latencyTier)));
    }
    ...
    if (!response.ok) {
      const error = await response.text();
      throw new Error(`ElevenLabs TTS failed: ${response.statusText} - ${error}`);
    }
  ...
  async *streamSynthesize(text: string, options?: TTSOptions): AsyncGenerator<Buffer> {
    ...
    if (!response.ok) {
      const error = await response.text();
      throw new Error(`ElevenLabs TTS streaming failed: ${response.statusText} - ${error}`);
    }
    ...
    for (const [, voiceId] of this.clonedVoiceCache) {
      deletePromises.push(
        this.deleteClonedVoice(voiceId).catch((err: Error) => {
          this.logger?.warn(`[ElevenLabs] Failed to delete cloned voice ${voiceId} on shutdown: ${err.message}`);
        }),
      );
    }
...
const plugin: WOPRPlugin & {
  ...
  getWebMCPHandlers(): Record<string, (input: Record<string, unknown>) => Promise<unknown>>;
} = {
  name: PLUGIN_ID,
  version: "1.0.0",
  description: "ElevenLabs high-quality text-to-speech",
  ...
  async init(pluginCtx: WOPRPluginContext) {
    ...
    _provider.fetchVoices().catch((err: unknown) => {
      ctx?.log.warn("Failed to fetch ElevenLabs voices on startup:", err instanceof Error ? err.message : String(err));
    });
    ...
    type CtxWithCapabilityProvider = {
      registerCapabilityProvider(type: string, provider: { id: string; name: string }): void;
      unregisterCapabilityProvider?(type: string, id: string): void;
    };
    if (
      "registerCapabilityProvider" in pluginCtx &&
      typeof (pluginCtx as unknown as CtxWithCapabilityProvider).registerCapabilityProvider === "function"
    ) {
      ...
      try {
        ...
        cleanups.push(() => {
          if (
            ...
            typeof (ctx as unknown as CtxWithCapabilityProvider).unregisterCapabilityProvider === "function"
          ) {
            (ctx as unknown as CtxWithCapabilityProvider).unregisterCapabilityProvider?.(
              "tts",
        ...
        ctx?.log.warn("Failed to register TTS capability provider:", err instanceof Error ? err.message : String(err));
      }
    }
  },
