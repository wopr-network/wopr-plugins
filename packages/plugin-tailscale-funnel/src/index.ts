    ...
    if (typeof ctx.registerA2AServer === "function") {
      ctx.registerA2AServer({
        ...
        tools: [
          {
            name: "funnel_status",
            description: "Get Tailscale Funnel status: enabled/disabled, public URL, tailnet hostname.",
          ...
          },
          {
            ...
            handler: async () => ({
              content: [
                {
                  type: "text" as const,
                  text: JSON.stringify(buildNodeStatusResponse(getTailscaleStatusJson(), available ?? false, hostname)),
                },
              ],
            }),
          },
