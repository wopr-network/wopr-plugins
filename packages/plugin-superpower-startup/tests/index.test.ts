...
describe("wopr-plugin-superpower-startup", () => {
  ...
  describe("plugin metadata", () => {
    it("has correct name", () => {
      expect(plugin.name).toBe("wopr-plugin-superpower-startup");
    });
  ...
  describe("manifest", () => {
    ...
    it("declares dependencies on skills, memory-semantic, cron", () => {
    ...
    it("has setup steps", () => {
      expect(plugin.manifest?.setup).toHaveLength(2);
      expect(plugin.manifest?.setup?.[0].id).toBe("welcome");
      expect(plugin.manifest?.setup?.[1].id).toBe("startup-context");
    });
  ...
  describe("init()", () => {
    ...
    it("context provider returns persona with system role", async () => {
      ...
      expect(result).toEqual({
        content: expect.stringContaining("co-founder"),
        role: "system",
  ...
  describe("shutdown()", () => {
    it("unregisters context provider after init", async () => {
      ...
      expect(mockCtx.unregisterContextProvider).toHaveBeenCalledWith("superpower-startup");
    });
