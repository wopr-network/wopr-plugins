...
import type { AudioFormat, TTSOptions, TTSProvider, TTSSynthesisResult, Voice, VoicePluginMetadata } from "wopr/voice";
...
class OpenAITTSProvider implements TTSProvider {
  ...
  validateConfig(): void {
    ...
    if (!validVoices.includes(this.defaultVoice)) {
      throw new Error(`Invalid voice: ${this.defaultVoice}. Valid: ${validVoices.join(", ")}`);
    }
    ...
    if (!validModels.includes(this.model)) {
      throw new Error(`Invalid model: ${this.model}. Valid: ${validModels.join(", ")}`);
    }
  }
  ...
  configSchema: {
    ...
    fields: [
      ...
      },
      {
        ...
        type: "text",
        label: "TTS Model",
        description: "TTS model: gpt-4o-mini-tts (recommended), tts-1, tts-1-hd",
        required: false,
        setupFlow: "none",
      },
      {
...
const plugin: WOPRPlugin = {
  ...
  async init(ctx: WOPRPluginContext) {
    ...
    try {
      ...
      ctx.registerConfigSchema("@wopr-network/wopr-plugin-voice-openai-tts", manifest.configSchema);
      ctx.log.info(
  ...
  async shutdown() {
    ...
    pluginCtx?.unregisterConfigSchema("@wopr-network/wopr-plugin-voice-openai-tts");
    provider = null;
    pluginCtx = null;
  },
};
