...
import type { PluginCommand, WOPRPlugin, WOPRPluginContext } from "@wopr-network/plugin-types";
  ...
  return (
    ...
    voices.every(
      (voice) =>
        ...
        ((voice as { name?: unknown }).name === undefined || typeof (voice as { name?: unknown }).name === "string") &&
...
function firstProvider<T>(providers: unknown[] | undefined, guard: (v: unknown) => v is T): T | null {
    ...
    async handler(ctx: WOPRPluginContext, args: string[]) {
      ...
      switch (subcommand) {
        ...
        default:
          ...
          ctx.log.info("Usage: wopr voice <transcribe|synthesize|list|providers>");
      }
    },
  },
];
 ...
 */
async function transcribeCommand(ctx: WOPRPluginContext, args: string[]): Promise<void> {
  const { flags, positional } = parseFlags(args);
  const [inputFile] = positional;

  if (!inputFile) {
    ctx.log.error("Usage: wopr voice transcribe <audio-file> [--output transcript.txt]");
    return;
  }
  ...
  ctx.log.info(`Transcribing ${basename(inputFile)} using ${stt.metadata.name}...`);
  ...
  try {
    ...
    ctx.log.error(`Transcription failed: ${err instanceof Error ? err.message : String(err)}`);
  }
}
 ...
 */
async function synthesizeCommand(ctx: WOPRPluginContext, args: string[]): Promise<void> {
  const { flags, positional } = parseFlags(args);
  const [voice, ...textParts] = positional;
  const text = textParts.join(" ");

  if (!voice || !text) {
    ctx.log.error('Usage: wopr voice synthesize <voice> "<text>" [--output file.pcm]');
    ctx.log.info("\nExamples:");
    ctx.log.info('  wopr voice synthesize coral "Hello world"');
    ctx.log.info('  wopr voice synthesize alloy "Welcome" --output greeting.pcm');
    return;
  }
  ...
  try {
    ...
    ctx.log.info(`Generated ${result.audio.length} bytes of ${result.format} audio`);
    ...
    ctx.log.error(`Synthesis failed: ${err instanceof Error ? err.message : String(err)}`);
  }
}
...
async function providersCommand(ctx: WOPRPluginContext): Promise<void> {
  ...
  ctx.log.info("\nSTT (Speech-to-Text):");
  if (stt) {
    ctx.log.info(`  ${stt.metadata.emoji || "🎤"} ${stt.metadata.name} v${stt.metadata.version}`);
  ...
  ctx.log.info("\nTTS (Text-to-Speech):");
  if (tts) {
    ctx.log.info(`  ${tts.metadata.emoji || "🔊"} ${tts.metadata.name} v${tts.metadata.version}`);
...
const plugin: WOPRPlugin = {
  ...
  async init(ctx: WOPRPluginContext) {
    ctx.log.info("Voice CLI commands registered: wopr voice <transcribe|synthesize|list|providers>");
  },
};
