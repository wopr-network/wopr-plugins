 ...
 */
export function registerImessageTools(registry: WebMCPRegistry, apiBase = "/api"): void {
  // 1. getImessageStatus
  registry.register({
  ...
  registry.register({
    ...
    description: "List active iMessage conversations with chat IDs and display names.",
    parameters: {
      limit: {
        type: "number",
  ...
  registry.register({
    ...
    description: "Get iMessage processing statistics: queued messages and active conversation count.",
    parameters: {},
    handler: async (_params: Record<string, unknown>, auth: AuthContext) => {
