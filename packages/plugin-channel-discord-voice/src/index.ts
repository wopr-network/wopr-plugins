...
const commands = [
  ...
  new SlashCommandBuilder().setName("voice-leave").setDescription("Leave the voice channel"),
  new SlashCommandBuilder().setName("voice-status").setDescription("Show voice channel status"),
];
...
async function transcribeUserSpeech(guildId: string, userId: string): Promise<void> {
  ...
  const stt = ctx.getCapabilityProviders("stt")[0] as unknown as STTExtension | undefined;
...
async function playTTSResponse(guildId: string, text: string): Promise<void> {
  ...
  const tts = ctx.getCapabilityProviders("tts")[0] as unknown as TTSExtension | undefined;
...
async function registerSlashCommands(token: string, clientId: string, guildId?: string): Promise<void> {
  ...
  try {
    ...
    if (guildId) {
      ...
      const existingCommands = (await rest.get(Routes.applicationGuildCommands(clientId, guildId))) as {
        name: string;
      }[];

      // Filter out our voice commands from existing (in case of re-registration)
      const otherCommands = existingCommands.filter((cmd) => !voiceCommandNames.has(cmd.name));
      ...
      const existingCommands = (await rest.get(Routes.applicationCommands(clientId))) as { name: string }[];
...
const plugin: WOPRPlugin & {
  ...
  async init(context) {
    ...
    cleanups.push(() => ctx?.unregisterConfigSchema?.("wopr-plugin-channel-discord-voice"));
    ...
    if (daveEnabled) {
      ...
      logger.warn("DAVE end-to-end encryption DISABLED - voice connections will fail after March 1, 2026");
    }
