...
export function extractToken(req: IncomingMessage, url?: URL): ExtractTokenResult {
  ...
  // Custom header
  const headerToken = typeof req.headers["x-wopr-token"] === "string" ? req.headers["x-wopr-token"].trim() : "";
  if (headerToken) {
...
export function validateAgentPayload(
  ...
  const channel = typeof payload.channel === "string" && payload.channel.trim() ? payload.channel.trim() : undefined;
  ...
  const timeoutSeconds =
    typeof payload.timeoutSeconds === "number" && Number.isFinite(payload.timeoutSeconds) && payload.timeoutSeconds > 0
...
export async function handleAgent(
  ...
  const { message, name, sessionKey, wakeMode, deliver, channel, to, model, thinking, timeoutSeconds } =
    validated.value;
...
async function executeAction(action: HookAction, ctx: WebhookHandlerContext): Promise<WebhookResponse> {
  if (action.kind === "wake") {
    ctx.logger.info({
...
async function runAgentAsync(config: AgentRunConfig, ctx: WebhookHandlerContext): Promise<void> {
  ...
  const safeMessage = allowUnsafeExternalContent ? message : wrapExternalContent(message, name || "webhook");
...
export async function handleGitHub(
  ...
  const org =
    ...
    (((payload.repository as Record<string, unknown> | undefined)?.owner as Record<string, unknown> | undefined)
