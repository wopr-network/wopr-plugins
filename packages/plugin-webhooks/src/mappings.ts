...
export function resolveMappings(config: WebhooksConfig, woprHome: string): HookMappingResolved[] {
  ...
  const transformsDir = config.transformsDir ? resolvePath(woprHome, config.transformsDir) : woprHome;

  return mappings.map((m, i) => normalizeMapping(m, i, transformsDir));
}

function normalizeMapping(mapping: HookMappingConfig, index: number, transformsDir: string): HookMappingResolved {
...
function mergeAction(
  ...
  if (kind === "wake") {
    ...
    const session = typeof override.session === "string" ? override.session : (baseWake?.session ?? "");
...
async function loadTransform(transform: { modulePath: string; exportName?: string }): Promise<HookTransformFn> {
