...
import { clearDeliveryHistory, createWebhooksExtension, recordDelivery } from "./webhooks-extension.js";
...
function resolveConfig(config: WebhooksConfig, woprHome: string): WebhooksConfigResolved | null {
  ...
  const maxBodyBytes = config.maxBodyBytes && config.maxBodyBytes > 0 ? config.maxBodyBytes : DEFAULT_MAX_BODY_BYTES;
...
const plugin: WOPRPlugin = {
  ...
  async init(pluginCtx: WOPRPluginContext) {
    ...
    const extension: WebhooksExtension = {
      ...
      async testMapping(mappingId: string, payload: Record<string, unknown>): Promise<HookMappingResult | null> {
        if (!resolvedConfig) {
          return null;
        }
    ...
    const agentResponseHandler = async (woprEvent: { type: string; payload: unknown }) => {
      ...
      const p = (woprEvent.payload && typeof woprEvent.payload === "object" ? woprEvent.payload : {}) as Record<
        string,
        unknown
      >;
