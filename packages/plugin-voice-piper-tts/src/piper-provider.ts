...
import type { TTSOptions, TTSProvider, TTSSynthesisResult, Voice, VoicePluginMetadata } from "./voice-types.js";
...
export class PiperTTSProvider implements TTSProvider {
  ...
  validateConfig(): void {
    ...
    if (!voiceExists) {
      throw new Error(`Invalid voice: ${this.config.voice}. Use one of: ${this.voices.map((v) => v.id).join(", ")}`);
    }

    const validRates = [16000, 22050, 24000, 48000];
    if (!validRates.includes(this.config.sampleRate)) {
      throw new Error(`Invalid sample rate: ${this.config.sampleRate}. Valid: ${validRates.join(", ")}`);
    }
  ...
  private async runPiperContainer(
    ...
    const container = await this.docker.createContainer({
      ...
      StdinOnce: true,
      HostConfig: {
        Binds: [`${modelDir}:/models`, `${inputFile}:/input/input.txt:ro`, `${join(outputFile, "..")}:/output`],
        AutoRemove: true,
      },
    });
  ...
  private async pullImage(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      (docker as any).pull(this.config.image, (error: unknown, stream: NodeJS.ReadableStream) => {
        if (error) {
          reject(error);
          return;
        }
